...
class EdgeNode
  ...
  def initialize(y, weight, next)
    @y = y if y.is_a?(Integer)
    @weight = weight if weight.is_a?(Integer)
    @next = next if next.is_a?(EdgeNode)
  end
end
...
class Graph
  ...
  # if we want to do something else, these are where we do it.
  # def process_vertex_early(vertex, string = "processed vertex #{vertex}")
  #   return puts string
  # end
  #
  ...
  # end
  #
  # def process_edge(vertex, y, string = "processed edge #{vertex},#{y}")
  #   return puts string
  # end

  # basic pathfinding!
  # will return a string containing the shortest path to a thing
  ...
  # connected components! we can find everything that's connected in a graph.
  ...
  # this will use BFS to label each vertex of the array.
  def label_two_color
  ...
  # overwrites another method, don't load this class.
